...
def on_session_started(session_started_request, session):
    return build_response({"topic": "none", "index": -1, "last_quote": "none"}, build_speechlet_response("", "", "", False))


def on_launch(launch_request, session):
    return build_response({"topic": "none", "index": -1, "last_quote": "none"}, build_speechlet_response("Welcome.", "Welcome. How can I help you?", "If you want to be motivated, just ask me to tell you a quote.", False))
...
def list_quotes(topic, session):
    ...
    return build_response({"topic": topic, "index": -1, "last_quote": "none"}, build_speechlet_response("Quotes", context, "", True))
...
def tell_one_quote(topic, session):
    ...
    return build_response({"topic": topic, "index": index, "last_quote": quotes[index]}, build_speechlet_response("Quote", context, "Would you like a new one? Or do you want to do something with this quote?", False))
...
def send_to_email(session):
    ...
    return build_response({"topic": session['attributes']['topic'], "index": session['attributes']['index'], "last_quote": session['attributes']['last_quote']}, build_speechlet_response("Success", "I successfully shared the quote.", "I'm still here", False))